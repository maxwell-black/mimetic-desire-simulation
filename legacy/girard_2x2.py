"""
Girard 2Ã—2 Convergence Module
============================

Implements the paper's core 2Ã—2 design:
...
class GirardConfig:
    ...
    sigma_status: float = 0.10       # sigma_S in f(Î”S)=exp(-Î”S/sigma_S)
    beta_up: float = 1.0             # beta_â†‘ in UpwardBias
    c_status: float = 0.50           # prestige baseline c_status in w âˆ w0(c_status+S_k)
    status_init_low: float = 0.4
    status_init_high: float = 0.6
    status_loss_rate: float = 0.005  # Î» (max per-step status loss at current most-targeted agent)
...
class GirardSimulation:
    """
    Core simulator for the Girard 2Ã—2 design.
    ...
    def __init__(self, cfg: GirardConfig, source: SourceMode, spread: SpreadMode):
        ...
        # History (lightweight; extended metrics computed later as needed)
    ...
    def _refresh_prestige(self) -> None:
        ...
        For status-source variants: w_ik(t) âˆ w0_ik * (c_status + S_k(t)).
    ...
    def step_expulsion(self) -> None:
        ...
        if received[most_targeted] >= cfg.expulsion_threshold:
            ...
            # Zero hostility toward expelled agent
            for other in self._alive_ids():
                self.aggression[other][most_targeted] = 0.0
            # Optional: zero the expelled agent's own aggression row (not used since dead)
...
def run_many(
    ...
    for r in range(n_runs):
        sim_cfg = replace(cfg, seed=base_seed + r)
