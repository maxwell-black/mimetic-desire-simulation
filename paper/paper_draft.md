...
An ablation control (Appendix C1) confirms that this result depends on the redistributive budget structure specifically, not on convexity of response alone. Replacing the AC operator with a fixed-scale convex map---$\text{pull}_i(j) = C \cdot h_i(j)^\gamma$, with $C$ calibrated from a linear burn-in to match total hostility throughput---yields no stable convergence regime. Below a sharp explosion threshold $C_{\mathrm{crit}}$, the system behaves identically to the linear baseline (peak modal agreement $\approx 0.10$). Above it, total tension diverges within 25--75 steps. The per-step L1-conserving renormalization is therefore constitutive of convergence: it bounds total mimetic pull at $\sum_k h_i(k)$ while redistributing that fixed budget toward the leading target, creating the zero-sum cross-target competition that no fixed-scale map can replicate.
