...
We define *convergence* as modal-target agreement >= 0.95 sustained for 10 consecutive steps. The theoretical ceiling is $(N-1)/N = 0.98$, since self-targeting is excluded. The 0.95 threshold is somewhat arbitrary; we verified that alternative thresholds of 0.90 and 0.98 do not meaningfully shift the gamma phase boundary identified in Section 3.2.
...
*Table 1. Summary metrics across 10 runs per variant, alpha = 0.15, salience exponent = 2.0, 600 timesteps. Gini, top-target share, and convergence ratio are time-averaged over all 600 timesteps within each run (note that this averaging mixes pre- and post-convergence regimes for AC and RA; peak and final-window values are reported in the gamma sweep, Table 2). Expulsions and catharsis are cumulative per run.*

The results divide cleanly along the transmission-character axis. Variants with linear transmission (LM, RL) produce Gini coefficients around 0.11--0.13, top-target shares near $1/N$ (uniform baseline for 50 agents is 0.02), and convergence ratios near 1.0. Variants with convex redistributive transmission (AC, RA) produce Gini above 0.73, top-target shares of 0.31--0.35, and convergence ratios of 1.4--1.6. Adding rivalry dynamics to linear mimesis (LM to RL) produces negligible change. Adding them to attentional concentration (AC to RA) increases Gini by ~8% and deepens catharsis, but the redistributive mechanism does the overwhelming majority of the convergence work.
...
[^C1]: **Note:** Appendix C1 (fixed-scale convex map ablation) is referenced in Sections 3.3 and 4.1 but is not yet included in this draft. It should report the $C$-sweep results showing the absence of a stable convergence regime between the linear-baseline and explosion conditions.
