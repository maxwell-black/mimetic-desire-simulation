...
[^clip]: Clamping hᵞ to [0, max-val] produces results identical to Condition 2, confirming the collapse is due to signal attrition, not numerical overflow.
...
[^k-note]: The ultra-long runs at N = 500 (`exp1_ultralong_n500.py`) used a fixed or differently-computed k value. A rerun with the adaptive k = max(6, min(⌊0.12N⌋, 20)) specification used in the interpolation experiments is planned.
...
[^n10]: This discretization effect is specific to the convergence *criterion*, not the mechanism. The adaptive threshold introduced in Section 3.11.1 (θ(N) = min(0.95, (N−1)/N)) would register N = 10 convergence at θ = 0.90, but was not applied in these softmax experiments, which use the fixed 0.95 threshold for consistency with the power-law sweep.
